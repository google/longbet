...
for (pr in pr_types){
  for (trt in trt_types){
    ...
    for (iter in seq(20, num_sweeps, by = 20)){
      longbet.att <- longbet.catt.sweeps[,,1:iter] %>%
      ...
      longbet.catt <- apply(longbet.catt.sweeps[,,1:iter], c(1, 2), mean, na.rm = T)
      longbet.catt.low <- apply(longbet.catt.sweeps[,,1:iter], c(1, 2), quantile, prob = alpha /2 , na.rm = T)
      longbet.catt.high <- apply(longbet.catt.sweeps[,,1:iter], c(1, 2), quantile, prob = 1 - alpha /2 , na.rm = T)
      
      att.results[nrow(att.results) + 1,] <- c(pr, trt, iter, att.metric(att, longbet.att))
      catt.results[nrow(catt.results) + 1, ] <- c(pr, trt, iter, catt.metric(align_tau, longbet.catt, longbet.catt.low, longbet.catt.high))
    }
  }
}

write.csv(att.results, 'sweeps_sim.csv', row.names = F)


att.results <- read.csv('sweeps_sim.csv')
att.results %>%
  select(-c(Bias, Cover0, I.L)) %>%
  gather("metric", "value", -pr, -trt, -sweeps) %>% 
  mutate_at(c('sweeps', 'value'), as.numeric) %>%
  mutate(across(where(is.numeric), ~ round(., 3))) %>%
  ggplot(aes(x = sweeps, y = value, group = interaction(pr, trt), color = interaction(pr, trt))) +
  geom_line() + 
