...
double squared_exponential(double & x, double &y,
double &sigma2, double &lambda2){
    // calculate squared exponential kernel with param sigma^2 andn lambda^2
    return sigma2 * exp( - pow(x - y, 2) / lambda2 / 2);
}
...
void get_unique_values(std::vector<double> &vec,
std::vector<double> &unique_vec){
    unique_vec.resize(vec.size());
    // using default comparison:
    std::vector<double>::iterator it;
    it = std::unique_copy (vec.begin(), vec.end(), unique_vec.begin());
    std::sort(unique_vec.begin(), it);
    // using predicate comparison:
    it = std::unique_copy(unique_vec.begin(), it, unique_vec.begin(), equal);

    unique_vec.resize(std::distance(unique_vec.begin(), it));
}

int get_index(std::vector<double> v, double K)
{
