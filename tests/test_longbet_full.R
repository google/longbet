...
#### 2. longBet
# run longBet
## TODO: change longBet to longBet_full
longbet.fit <- longBet(y_vec, z_vec, x_con, x_mod, t_vec, pi_vec, 
n_trees_con =  0, n_trees_mod = 40,
alpha_mod = 0.95, beta_mod = 1.25,
pcat_con = 5,  pcat_mod = 5)

# get treatment individual-level estimates
tauhats <- getTaus(longbet.fit)
betahats <- longbet.fit$beta_draws
print(dim(betahats))

# main model parameters can be retrieved below
#print(longbet.fit$model_params)
